...
export default function AllGiftBoxingPage() {
  ...
  const handleAddToCart = async (item: typeof CATALOG_PRODUCTS[0], e: React.MouseEvent) => {
    ...
    await addItem({
      productId: item._id,
      giftBoxingType: item.segmentId || 'classics',
      quantity: 1,
      name: item.name,
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-[#FAF7F2] pb-28 md:pb-20 font-sans">
      {/* 1. Category Story Circles Header */}
      <section className="pt-6 pb-6 px-4 md:px-8 border-b border-[#E4E0D7]/60">
        <div className="max-w-7xl mx-auto space-y-5 text-center">
          <h1 className="font-serif-luxury text-xl md:text-3xl font-bold tracking-wider uppercase text-[#2C3228]">
            ALL THINGS GIFTING
          </h1>

          {/* Swipeable Category Circles */}
          <div className="flex items-center justify-start sm:justify-center gap-6 sm:gap-10 md:gap-14 overflow-x-auto no-scrollbar py-2 px-2">
            {CATEGORY_CIRCLES.map((cat) => {
              ...
              return (
                <button
                  key={cat.id}
                  onClick={() => setSelectedFilter(selectedFilter === cat.id ? 'all' : cat.id)}
          ...
          {filteredProducts.map((item) => {
            ...
            return (
              <div
                ...
                className="group relative bg-white rounded-none border border-[#E4E0D7] flex flex-col justify-between shadow-2xs hover:shadow-md transition-all duration-300 cursor-pointer text-center overflow-hidden"
              >
                ...
                {/* Content Section At Bottom (Centered Text, No CRAFTED IN-HOUSE tag) */}
                <div className="p-3 sm:p-4 space-y-2 flex-1 flex flex-col justify-between items-center text-center bg-white rounded-none">
                  ...
                  <div className="w-full flex items-center justify-between pt-2.5 border-t border-[#E4E0D7]/50 mt-2">
